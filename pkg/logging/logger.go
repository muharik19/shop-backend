...
func Configure() {
	...
	lvl := strings.ToLower(util.Getenv("LOG_LEVEL"))
	switch lvl {
	case "debug":
...
func LogRequest(user string, r *http.Request, body string) {
	logrus.SetFormatter(loggerFormat())
...
func LogResponse(user string, response string) {
	logrus.SetFormatter(loggerFormat())
